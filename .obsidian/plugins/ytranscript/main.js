...
var YoutubeTranscript = class {
  ...
  static generateRequest(page, config) {
    ...
    const clickTrackingParams = (_d = page == null ? void 0 : page.split('"clickTrackingParams":"')[1]) == null ? void 0 : _d.split('"')[0];
    return {
      context: {
        client: {
      ...
      filteredBlocks.forEach((block) => {
        ...
        blockContainerEl.addEventListener("contextmenu", (event) => {
          ...
          menu.addItem((item) => item.setTitle("Copy all").onClick(() => {
            navigator.clipboard.writeText(data.lines.map((t) => t.text).join(" "));
          }));
          menu.showAtPosition({
            x: event.clientX,
            y: event.clientY
